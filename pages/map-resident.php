      ...
      <body>
        <div class="map-flex-container">
            ...
            <div class="cont-map-right">
                  <div class="map-right-mini">
                        <div class="right-img" >
                            <img id="userpopup" src="../assets/people.png">
                        </div>

                        <div class="right-headings">
                            <h1 id="peoplename">
                                    <?php 
                                        $srchfname = $_SESSION['find_user_fname'];
                                        $srchlname = $_SESSION['find_user_lname'];

                                        $sql = "SELECT ResidentID, Firstname, Lastname, Middlename, Suffix
                                        FROM residents 
                                        WHERE Firstname = '$srchfname' AND Lastname = '$srchlname'";
                                        ...
                                        if ($result->num_rows > 0) {
                                            // output data of each row
                                            while($row = $result->fetch_assoc()) {
                                              $_SESSION['rsdntID'] = $row["ResidentID"];
                                                echo $row["Lastname"] . ", " . $row["Firstname"] . " " . $row["Middlename"] . " " . $row["Suffix"];
                                                $_SESSION['search_fname'] = $row["Firstname"];
                                                $_SESSION['search_lname'] = $row["Lastname"];
                                            }
                            ...
                            <table>
                              <tr>
                                ...
                                <th>Government <br>Program</th>
                                          ...
                                          <?php
                                            $query = "SELECT Housenumber, householdnumber, birthdate, age, address, contactnumber, programs.ProgramName
                                              FROM residents
                                            ...
                                            if ($result->num_rows > 0) {
                                              while($row = $result->fetch_assoc()) {
                                                echo "<tr><td>".$row["Housenumber"]."</td><td>".$row["householdnumber"]."</td><td> ".$row["birthdate"]."</td><td>".$row["age"]."</td><td>".$row["address"]."</td><td>".$row["contactnumber"]."</td><td>".$row["ProgramName"];
                        ...
                        <div style="margin-left: 60px;">
                            <a href="notify-page.php"><button id="mprs-submit" style="margin-top: 40px; margin-left: 120px;">NOTIFY</button></a>
                            <button id="mprs-submit" onclick="openForm()" style="margin: 1%;">DETAILS</button>
                        </div>
                  </div>
            </div>
            ...
            <div class="form-popup" id="myForm">
                <form action="" class="form-container">
                      <table>
                          <tr>
                            ...
                            <th>Relationship</th>
                                ...
                                <?php
                                $myrsdntID = $_SESSION['rsdntID'];
                                    $query1 = "SELECT housemembers.Lastname, housemembers.FirstName, housemembers.MiddleName, housemembers.Suffix, Birthdate, RELATIONS.Relationship
                                    FROM residents
                                    INNER JOIN housemembers ON housemembers.ResidentID = residents.ResidentID
                                    INNER JOIN relations ON relations.RelationID = housemembers.RelationID
                                    WHERE housemembers.residentID = '$myrsdntID';";
                                    $results = $conn->query($query1);
                                            
                                    if ($results->num_rows > 0) {
                                       while($row = $results->fetch_assoc()) {
                                           echo "<tr><td>".$row["Lastname"].", ".$row["FirstName"]." ".$row["MiddleName"]." ".$row["Suffix"]."</td><td>".$row["Birthdate"]."</td><td>".$row["Relationship"];
                  ...
                  <button class="popup-submit" onclick="closeForm()">Close</button>
                </form>
            </div>
            ...
            function closeForm() {
              document.getElementById("myForm").style.display = "none";
                ...
                <?php 
                  ...
                  $mysearch_fname = $_SESSION['search_fname'];
                  $mysearch_lname = $_SESSION['search_lname'];
                  ...
                  $sql = "INSERT INTO searchlog(SearchLogID, ResidentID, Lastname, Firstname, DateSearch, AdminID)
                          VALUES ('', '$myrsdntID', '$mysearch_lname', '$mysearch_fname', '$time', '$myadminID'); ";
                          $result = mysqli_query($conn,$sql);
                ?>
      </body>
</html>